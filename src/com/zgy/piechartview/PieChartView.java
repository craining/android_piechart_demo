		...
		if (itemsAngle != null && itemsBeginAngle != null) {
			...
			for (int i = 0; i < itemsAngle.length; i++) {
				if (itemPostion == i && !isRotating) {
					 Log.e(TAG, "draw last  separateDistence=" + separateDistence);
					switch (rotateWhere) {
					case TO_RIGHT:
						// 向右移动
					...
					case NO_ROTATE :
						...
						//中线角度  = itemsBeginAngle[i] + itemsAngle[i]/2
						//根据中线角度判断新矩形位置
						//若 0 < angle < 90
					...
					Log.e(TAG, "leftTop=" + leftTop + "   rigthBottom="  + rigthBottom);
